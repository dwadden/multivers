...
# TODO(dwadden) Write a quick test for the collator?
...
class FactCheckingReader:
    def __init__(self, debug=False, negative_samples=0):
        ...
        self.negative_sample_root = (
            util.data_dir / "negative_samples/claims_fewer_negatives"
        )
...
class ExternalReader(FactCheckingReader):
    def get_fold(self, fold, tokenizer):
        ...
        if isinstance(self, PubMedQAReader):
            data_file = f"{self.data_dir}/{fold_name}_sentbert.jsonl"
        else:
